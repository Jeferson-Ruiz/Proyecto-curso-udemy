...
public class MsvcGatewayServerApplication {
	...
	RouterFunction<ServerResponse> routerConfig(){
		return route("msvc-products")
			...
			.filter((request, next) -> {
				ServerRequest requestModified = ServerRequest.from(request).header("message-request", "mensaje personalizado request").build();

				ServerResponse response = next.handle(requestModified);
				response.headers().add("message-response", "mesaje respuesta");
				return response;
			})
		...
		.setId("products")
		.setStatusCodes("500")
		.setFallbackPath("forward:/api/items/1")))
